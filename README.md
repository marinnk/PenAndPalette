...
小説（テキスト作品）とイラストの両方を投稿でき、他利用者の作品に対して「いいね」「コメント」に加えて、「この小説にイラストを描かせてほしい」「このイラストのキャラクターで小説を書かせてほしい」といったリクエストを送り合える点が特徴です。姉妹プロジェクト [RaiseTechSNS](../RaiseTechSNS) と同じインフラ構成・開発フローを踏襲しつつ、言語（Java/Spring → Python/Django）と DB（PostgreSQL → MySQL）を変えて実装しています。
