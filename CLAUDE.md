...
- 同じ表示・処理を複数箇所に書きそうになったら、書く前に共通化する（実装後の`/simplify`任せにしない）
  - フロントエンド：アイコン画像表示のような、複数の画面・コンポーネントで見た目が同じ表示要素は、最初から共通コンポーネント（例:`components/AvatarIcon.vue`）として切り出す。ボタンや入力欄などの基本要素も、2箇所目を書く時点で共通化を検討する
  - バックエンド：「S3アップロード失敗を握りつぶしてログに残す」のような、特定のapp（`posts`・`users`等）に閉じない横断的な処理は`common/`配下に置き、各appからはそれを呼ぶだけにする
  - PRを作成する前に`/simplify`を実行し、今回の変更で生まれた重複が無いか確認してから作成する
